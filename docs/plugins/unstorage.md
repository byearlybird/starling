...
- `config.onBeforeSet` – Optional hook invoked before snapshots are persisted. Receives the Collection object in JSON:API format and must return the same structure. Use this for custom serialization or filtering.
